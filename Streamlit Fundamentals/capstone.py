...
def format_date_for_comparison(date: str) -> float:
    """Convert 'QX YYYY' into numeric year + fraction of quarter"""
    quarter, year = date.split()
    year = float(year)
    q_num = int(quarter[1])  # extract 1, 2, 3, or 4

    return year + (q_num - 1) * 0.25

@st.cache_data
def end_before_start(start_date, end_date):
    return format_date_for_comparison(start_date) > format_date_for_comparison(end_date)
